...
function bookingForm() {
  ...
  $(".booking-form form").each(function () {
    ...
    formBooking.on("submit", function (e) {
      ...
      const arrival = formBooking.find("input[name='date']").val().trim();
      if (!arrival) {
        formBooking
      ...
      const children =
        formBooking.find("input[name='child']").val().trim() || "0";
      if (!adult) {
        formBooking
...
function headerMobile() {
  ...
  const closeMenu = document.querySelectorAll(
    ".header-hambuger.close, .header-overlay-blur"
  );
